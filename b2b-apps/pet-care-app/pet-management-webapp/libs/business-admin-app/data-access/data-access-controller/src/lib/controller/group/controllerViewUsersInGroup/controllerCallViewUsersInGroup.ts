...
import { GroupList, UserList } from "@b2bsample/shared/data-access/data-access-common-models-util";
...
export async function controllerCallViewUsersInGroup(session: Session, group: string): Promise<UserList | null> {
    ...
    const data = (await commonControllerCall("/api/settings/group/viewUsersInGroup", 
        session, { param: group } ) as UserList | null);
